...
test.describe.serial("Account Migration Journal API Test", () => {
    ...
    test.describe("CRUD Operation Testing", () => {
        test("Add new Account Migration Journal", async ({ api }) => {
            ...
            apiObj.setUrl(
                `${accUrl}/api/GL?form=%27DM%27&Environment=qaa&AttachmentID=""`
            );
            ...
            if (payloadToSend.glDetails && payloadToSend.glDetails[0]) {
                ...
                detail1.Qty = createValues[21];
                detail1.UnitPrice = createValues[22];
                detail1.AccNumAccDesc = createValues[23];
                detail1.CCIDCodeCCIDDesc = createValues[24];
                detail1.rowid = createValues[25];
            }

            if (payloadToSend.glDetails && payloadToSend.glDetails[1]) {
                const detail2 = payloadToSend.glDetails[1];
                detail2.AccKey = createValues[26];
                detail2.AccNum = createValues[27];
                detail2.AccDesc = createValues[28];
                detail2.OUKey = createValues[29];
                detail2.OUCode = createValues[30];
                detail2.Remarks = createValues[31];
                detail2.FuncTransAmt = createValues[32];
                detail2.LocalTransAmt = createValues[33];
                detail2.OrigTransAmt = createValues[34];
                detail2.TaxableTransAmt = createValues[35];
                detail2.InclusiveTransAmt = createValues[36];
                detail2.OrigDrAmt = createValues[37];
                detail2.OrigCrAmt = createValues[38];
                detail2.DetRevAmt = createValues[39];
                detail2.CCIDKey = createValues[40];
                detail2.CCID = createValues[41];
                detail2.Qty = createValues[42];
                detail2.AccNumAccDesc = createValues[43];
                detail2.CCIDCodeCCIDDesc = createValues[44];
                detail2.rowid = createValues[45];
            }
