...
import {
  PayrollDeductionCodeSetupCreate,
  PayrollDeductionCodeSetupEdit1,
  PayrollDeductionCodeSetupEdit2,
  PayrollDeductionCodeSetupDelete,
} from "@UiFolder/pages/MasterFile/02_Payroll/07_PayrollDeductionCodeSetupPage";
...
const formName = "Payroll Deduction Code Setup";
...
test.describe.serial(`${formName} Tests`, () => {
  ...
  test(`Create ${formName}`, async ({ page, db }) => {
    ...
    const { uiVals, gridVals } = await PayrollDeductionCodeSetupCreate(
      page,
      sideMenu,
      paths,
      columns,
  ...
  test(`Edit ${formName} Without Saving`, async ({ page, db }) => {
    const { uiVals, gridVals } = await PayrollDeductionCodeSetupEdit1(
      page,
      sideMenu,
      paths,
      columns,
  ...
  test(`Edit ${formName} With Saving`, async ({ page, db }) => {
    const { uiVals, gridVals } = await PayrollDeductionCodeSetupEdit2(
      page,
      sideMenu,
      paths,
      columns,
  ...
  test(`Delete ${formName}`, async ({ page, db }) => {
    await PayrollDeductionCodeSetupDelete(page, sideMenu, editValues);
