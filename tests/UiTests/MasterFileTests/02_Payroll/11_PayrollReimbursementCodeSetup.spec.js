...
import {
  PayrollReimbursementCodeSetupCreate,
  PayrollReimbursementCodeSetupEdit1,
  PayrollReimbursementCodeSetupEdit2,
  PayrollReimbursementCodeSetupDelete,
} from "@UiFolder/pages/MasterFile/02_Payroll/11_PayrollReimbursementCodeSetupPage";
...
const formName = "Payroll Reimbursement Code Setup";
...
test.describe.serial(`${formName} Tests`, () => {
  ...
  test(`Create ${formName}`, async ({ page, db }) => {
    ...
    const { uiVals, gridVals } = await PayrollReimbursementCodeSetupCreate(
      page,
      sideMenu,
      paths,
      columns,
  ...
  test(`Edit ${formName} Without Saving`, async ({ page, db }) => {
    const { uiVals, gridVals } = await PayrollReimbursementCodeSetupEdit1(
      page,
      sideMenu,
      paths,
      columns,
  ...
  test(`Edit ${formName} With Saving`, async ({ page, db }) => {
    const { uiVals, gridVals } = await PayrollReimbursementCodeSetupEdit2(
      page,
      sideMenu,
      paths,
      columns,
  ...
  test(`Delete ${formName}`, async ({ page, db }) => {
    await PayrollReimbursementCodeSetupDelete(page, sideMenu, editValues);
