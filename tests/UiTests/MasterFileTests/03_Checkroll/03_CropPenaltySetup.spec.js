...
import {
  CropPenaltySetupCreate,
  CropPenaltySetupEdit1,
  CropPenaltySetupEdit2,
  CropPenaltySetupDelete,
} from "@UiFolder/pages/MasterFile/03_Checkroll/03_CropPenaltySetupPage";
...
const formName = "Crop Penalty Setup";
...
test.describe.serial(`${formName} Tests`, () => {
  ...
  test(`Create ${formName}`, async ({ page, db }) => {
    ...
    const { uiVals } = await CropPenaltySetupCreate(
      page,
      sideMenu,
      paths,
      columns,
  ...
  test(`Edit ${formName} Without Saving`, async ({ page, db }) => {
    const { uiVals } = await CropPenaltySetupEdit1(
      page,
      sideMenu,
      paths,
      columns,
  ...
  test(`Edit ${formName} With Saving`, async ({ page, db }) => {
    const { uiVals } = await CropPenaltySetupEdit2(
      page,
      sideMenu,
      paths,
      columns,
  ...
  test(`Delete ${formName}`, async ({ page, db }) => {
    await CropPenaltySetupDelete(page, sideMenu, editValues, ou);

    // Check if the CropPenalty Setup is deleted
    const dbValues = await db.retrieveData(masterSQLCommand(formName), {
      Code: editValues[0],
      OU: ou[0],
    });
