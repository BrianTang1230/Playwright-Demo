...
import {
  BuildingTypeSetupCreate,
  BuildingTypeSetupEdit1,
  BuildingTypeSetupEdit2,
  BuildingTypeSetupDelete,
} from "@UiFolder/pages/MasterFile/03_Checkroll/07_BuildingTypeSetupPage";
...
const formName = "Building Type Setup";
...
test.describe.serial(`${formName} Tests`, () => {
  ...
  test(`Create ${formName}`, async ({ page, db }) => {
    ...
    const { uiVals } = await BuildingTypeSetupCreate(
      page,
      sideMenu,
      paths,
      columns,
  ...
  test(`Edit ${formName} Without Saving`, async ({ page, db }) => {
    const { uiVals } = await BuildingTypeSetupEdit1(
      page,
      sideMenu,
      paths,
      columns,
  ...
  test(`Edit ${formName} With Saving`, async ({ page, db }) => {
    const { uiVals } = await BuildingTypeSetupEdit2(
      page,
      sideMenu,
      paths,
      columns,
  ...
  test(`Delete ${formName}`, async ({ page, db }) => {
    await BuildingTypeSetupDelete(page, sideMenu, editValues);

    // Check if the Building Type code is deleted
    const dbValues = await db.retrieveData(masterSQLCommand(formName), {
      Code: editValues[0],
    });
