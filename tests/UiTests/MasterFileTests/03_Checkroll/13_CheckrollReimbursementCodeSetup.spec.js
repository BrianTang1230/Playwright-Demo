...
import {
  CheckrollReimbursementCodeSetupCreate,
  CheckrollReimbursementCodeSetupEdit1,
  CheckrollReimbursementCodeSetupEdit2,
  CheckrollReimbursementCodeSetupDelete,
} from "@UiFolder/pages/MasterFile/03_Checkroll/13_CheckrollReimbursementCodeSetupPage";
...
const formName = "Checkroll Reimbursement Code Setup";
...
test.describe.serial(`${formName} Tests`, () => {
  ...
  test(`Create ${formName}`, async ({ page, db }) => {
    ...
    const { uiVals, gridVals } = await CheckrollReimbursementCodeSetupCreate(
      page,
      sideMenu,
      paths,
      columns,
  ...
  test(`Edit ${formName} Without Saving`, async ({ page, db }) => {
    const { uiVals, gridVals } = await CheckrollReimbursementCodeSetupEdit1(
      page,
      sideMenu,
      paths,
      columns,
  ...
  test(`Edit ${formName} With Saving`, async ({ page, db }) => {
    const { uiVals, gridVals } = await CheckrollReimbursementCodeSetupEdit2(
      page,
      sideMenu,
      paths,
      columns,
  ...
  test(`Delete ${formName}`, async ({ page, db }) => {
    await CheckrollReimbursementCodeSetupDelete(page, sideMenu, editValues);
