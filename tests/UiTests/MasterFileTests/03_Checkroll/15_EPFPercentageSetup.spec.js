...
import {
  EPFPercentageSetupCreate,
  EPFPercentageSetupEdit1,
  EPFPercentageSetupEdit2,
  EPFPercentageSetupDelete,
} from "@UiFolder/pages/MasterFile/03_Checkroll/15_EPFPercentageSetupPage";
...
const formName = "EPF Percentage Setup";
...
test.describe.serial(`${formName} Tests`, () => {
  ...
  test(`Create ${formName}`, async ({ page, db }) => {
    ...
    const { uiVals } = await EPFPercentageSetupCreate(
      page,
      sideMenu,
      paths,
      columns,
  ...
  test(`Edit ${formName} Without Saving`, async ({ page, db }) => {
    const { uiVals } = await EPFPercentageSetupEdit1(
      page,
      sideMenu,
      paths,
      columns,
  ...
  test(`Edit ${formName} With Saving`, async ({ page, db }) => {
    const { uiVals } = await EPFPercentageSetupEdit2(
      page,
      sideMenu,
      paths,
      columns,
  ...
  test(`Delete ${formName}`, async ({ page, db }) => {
    await EPFPercentageSetupDelete(page, sideMenu, editValues);

    // Check if the EPF Percentage Type code is deleted
    const dbValues = await db.retrieveData(masterSQLCommand(formName), {
      Code: editValues[0],
    });
