...
import {
  MachineryHarvestingSetupCreate,
  MachineryHarvestingSetupEdit1,
  MachineryHarvestingSetupEdit2,
  MachineryHarvestingSetupDelete,
} from "@UiFolder/pages/MasterFile/03_Checkroll/21_MachineryHarvestingSetupPage";
...
const formName = "Machinery Harvesting Setup";
...
test.describe.serial(`${formName} Tests`, () => {
  ...
  test(`Create ${formName}`, async ({ page, db }) => {
    ...
    const { uiVals } = await MachineryHarvestingSetupCreate(
      page,
      sideMenu,
      paths,
      columns,
  ...
  test(`Edit ${formName} Without Saving`, async ({ page, db }) => {
    const { uiVals } = await MachineryHarvestingSetupEdit1(
      page,
      sideMenu,
      paths,
      columns,
  ...
  test(`Edit ${formName} With Saving`, async ({ page, db }) => {
    const { uiVals } = await MachineryHarvestingSetupEdit2(
      page,
      sideMenu,
      paths,
      columns,
  ...
  test(`Delete ${formName}`, async ({ page, db }) => {
    await MachineryHarvestingSetupDelete(page, sideMenu, editValues, ou);

    // Check if the Machinery Harvesting Setup is deleted
    const dbValues = await db.retrieveData(masterSQLCommand(formName), {
      Code: editValues[0],
      OU: ou[0],
    });
