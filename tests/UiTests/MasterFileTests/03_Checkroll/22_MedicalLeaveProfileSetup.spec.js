...
import {
  MedicalLeaveProfileSetupCreate,
  MedicalLeaveProfileSetupEdit1,
  MedicalLeaveProfileSetupEdit2,
  MedicalLeaveProfileSetupDelete,
} from "@UiFolder/pages/MasterFile/03_Checkroll/22_MedicalLeaveProfileSetupPage";
...
test.describe.serial(`${formName} Tests`, () => {
  ...
  test(`Create ${formName}`, async ({ page, db }) => {
    ...
    const { uiVals, gridVals } = await MedicalLeaveProfileSetupCreate(
      page,
      sideMenu,
      paths,
      columns,
    ...
    await validateDBValues(gridVals.join(";").split(";"), gridDbColumns, gridDbValues[0]);
  });

  test(`Edit ${formName} Without Saving`, async ({ page, db }) => {
    const { uiVals, gridVals } = await MedicalLeaveProfileSetupEdit1(
      page,
      sideMenu,
      paths,
      columns,
  ...
  test(`Edit ${formName} With Saving`, async ({ page, db }) => {
    const { uiVals, gridVals } = await MedicalLeaveProfileSetupEdit2(
      page,
      sideMenu,
      paths,
      columns,
  ...
  test(`Delete ${formName}`, async ({ page, db }) => {
    await MedicalLeaveProfileSetupDelete(page, sideMenu, editValues, ou);
