...
import {
  RecoveryTypeSetupCreate,
  RecoveryTypeSetupEdit1,
  RecoveryTypeSetupEdit2,
  RecoveryTypeSetupDelete,
} from "@UiFolder/pages/MasterFile/03_Checkroll/24_RecoveryTypeSetupPage";
...
const formName = "Recovery Type Setup";
...
test.describe.serial(`${formName} Tests`, () => {
  ...
  test(`Create ${formName}`, async ({ page, db }) => {
    ...
    const { uiVals } = await RecoveryTypeSetupCreate(
      page,
      sideMenu,
      paths,
      columns,
  ...
  test(`Edit ${formName} Without Saving`, async ({ page, db }) => {
    const { uiVals } = await RecoveryTypeSetupEdit1(
      page,
      sideMenu,
      paths,
      columns,
  ...
  test(`Edit ${formName} With Saving`, async ({ page, db }) => {
    const { uiVals } = await RecoveryTypeSetupEdit2(
      page,
      sideMenu,
      paths,
      columns,
  ...
  test(`Delete ${formName}`, async ({ page, db }) => {
    await RecoveryTypeSetupDelete(page, sideMenu, editValues);
