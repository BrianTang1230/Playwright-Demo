...
import {
  WorkerBatchSetupCreate,
  WorkerBatchSetupEdit1,
  WorkerBatchSetupEdit2,
  WorkerBatchSetupDelete,
} from "@UiFolder/pages/MasterFile/03_Checkroll/27_WorkerBatchSetupPage";
...
const formName = "Worker Batch Setup";
...
test.describe.serial(`${formName} Tests`, () => {
  ...
  test(`Create ${formName}`, async ({ page, db }) => {
    ...
    const { uiVals } = await WorkerBatchSetupCreate(
      page,
      sideMenu,
      paths,
      columns,
  ...
  test(`Edit ${formName} Without Saving`, async ({ page, db }) => {
    const { uiVals } = await WorkerBatchSetupEdit1(
      page,
      sideMenu,
      paths,
      columns,
  ...
  test(`Edit ${formName} With Saving`, async ({ page, db }) => {
    const { uiVals } = await WorkerBatchSetupEdit2(
      page,
      sideMenu,
      paths,
      columns,
  ...
  test(`Delete ${formName}`, async ({ page, db }) => {
    await WorkerBatchSetupDelete(page, sideMenu, editValues, ou);

    // Check if the Worker Batch Setup is deleted
    const dbValues = await db.retrieveData(masterSQLCommand(formName), {
      Code: editValues[0],
      OU: ou[0],
    });
