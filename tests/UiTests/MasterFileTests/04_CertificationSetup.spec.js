...
test.describe.serial("Certification Setup Tests", () => {
  // ---------------- Before All ----------------
  ...
  // ---------------- Create Tests ----------------
  test("Create New Certification Setup", async ({ page, db }) => {
  ...
  test("Edit Certification Setup", async ({ page, db }) => {
    const { uiVals } = await CertificationSetupEdit(
      page,
      sideMenu,
      paths,
      columns,
  ...
  test("Delete Certification Setup", async ({ page, db }) => {
    ...
    if (dbValues.length > 0) {
      throw new Error("DB validation failed when deleting Certification Setup");
    }
  });
