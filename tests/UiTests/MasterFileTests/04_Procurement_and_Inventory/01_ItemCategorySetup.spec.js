...
import {
  ItemCategorySetupCreate,
  ItemCategorySetupEdit1,
  ItemCategorySetupEdit2,
  ItemCategorySetupDelete,
} from "@UiFolder/pages/MasterFile/04_Procurement_and_Inventory/01_ItemCategorySetupPage";
...
const formName = "Item Category Setup";
...
test.describe.serial(`${formName} Tests`, () => {
  ...
  test(`Create ${formName}`, async ({ page, db }) => {
    ...
    const { uiVals } = await ItemCategorySetupCreate(
      page,
      sideMenu,
      paths,
      columns,
  ...
  test(`Edit ${formName} Without Saving`, async ({ page, db }) => {
    const { uiVals } = await ItemCategorySetupEdit1(
      page,
      sideMenu,
      paths,
      columns,
  ...
  test(`Edit ${formName} With Saving`, async ({ page, db }) => {
    const { uiVals } = await ItemCategorySetupEdit2(
      page,
      sideMenu,
      paths,
      columns,
  ...
  test(`Delete ${formName}`, async ({ page, db }) => {
    await ItemCategorySetupDelete(page, sideMenu, editValues);

    // Check if the ItemCategory code is deleted
    const dbValues = await db.retrieveData(masterSQLCommand(formName), {
      Code: editValues[0],
    });
