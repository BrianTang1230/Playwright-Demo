...
import {
  ...
  VehicleSetupEdit2,
  VehicleSetupDelete,
} from "@UiFolder/pages/MasterFile/05_Vehicle/01_VehicleSetupPage";
...
const submodule = "Vehicle";
const formName = "Vehicle Setup";
...
test.describe.serial(`${formName} Tests`, () => {
  ...
  test(`Create ${formName}`, async ({ page, db }) => {
    ...
    const { uiVals } = await VehicleSetupCreate(
      page,
      sideMenu,
      paths,
      columns,
  ...
  test(`Edit ${formName} Without Saving`, async ({ page, db }) => {
    const { uiVals } = await VehicleSetupEdit1(
      page,
      sideMenu,
      paths,
      columns,
  ...
  test(`Edit ${formName} With Saving`, async ({ page, db }) => {
    const { uiVals } = await VehicleSetupEdit2(
      page,
      sideMenu,
      paths,
      columns,
  ...
  test(`Delete ${formName}`, async ({ page, db }) => {
    await VehicleSetupDelete(page, sideMenu, editValues, ou);

    // Check if the Vehicle Setup is deleted
    const dbValues = await db.retrieveData(masterSQLCommand(formName), {
      Code: editValues[0],
      OU: ou[0],
    });
