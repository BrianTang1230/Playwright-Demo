...
import {
  NurseryBatchCategorySetupCreate,
  NurseryBatchCategorySetupEdit1,
  NurseryBatchCategorySetupEdit2,
  NurseryBatchCategorySetupDelete,
} from "@UiFolder/pages/MasterFile/07_Nursery/02_NurseryBatchCategorySetupPage";
...
const submodule = "Nursery";
const formName = "Nursery Batch Category Setup";
...
test.describe.serial(`${formName} Tests`, () => {
  ...
  test(`Create ${formName}`, async ({ page, db }) => {
    ...
    const { uiVals } = await NurseryBatchCategorySetupCreate(
      page,
      sideMenu,
      paths,
      columns,
  ...
  test(`Edit ${formName} Without Saving`, async ({ page, db }) => {
    const { uiVals } = await NurseryBatchCategorySetupEdit1(
      page,
      sideMenu,
      paths,
      columns,
  ...
  test(`Edit ${formName} With Saving`, async ({ page, db }) => {
    const { uiVals } = await NurseryBatchCategorySetupEdit2(
      page,
      sideMenu,
      paths,
      columns,
  ...
  test(`Delete ${formName}`, async ({ page, db }) => {
    await NurseryBatchCategorySetupDelete(page, sideMenu, editValues);

    // Check if the Nursery Batch Category code is deleted
    const dbValues = await db.retrieveData(masterSQLCommand(formName), {
      Code: editValues[0],
    });
