...
import {
  NurseryBatchSetupCreate,
  NurseryBatchSetupEdit1,
  NurseryBatchSetupEdit2,
  NurseryBatchSetupDelete,
} from "@UiFolder/pages/MasterFile/07_Nursery/03_NurseryBatchSetupPage";
...
const formName = "Nursery Batch Setup";
...
test.describe.serial(`${formName} Tests`, () => {
  ...
  test(`Create ${formName}`, async ({ page, db }) => {
    ...
    const { uiVals } = await NurseryBatchSetupCreate(
      page,
      sideMenu,
      paths,
      columns,
  ...
  test(`Edit ${formName} Without Saving`, async ({ page, db }) => {
    const { uiVals } = await NurseryBatchSetupEdit1(
      page,
      sideMenu,
      paths,
      columns,
  ...
  test(`Edit ${formName} With Saving`, async ({ page, db }) => {
    const { uiVals } = await NurseryBatchSetupEdit2(
      page,
      sideMenu,
      paths,
      columns,
  ...
  test(`Delete ${formName}`, async ({ page, db }) => {
    await NurseryBatchSetupDelete(page, sideMenu, editValues, ou);

    // Check if the Nursery Batch Setup is deleted
    const dbValues = await db.retrieveData(masterSQLCommand(formName), {
      Code: editValues[0],
      OU: ou[0],
    });
