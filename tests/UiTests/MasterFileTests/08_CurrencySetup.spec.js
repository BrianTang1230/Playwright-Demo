...
test.describe.serial("Currency Setup Tests", () => {
  ...
  // ---------------- Create Tests ----------------
  test("Create New Currency Code", async ({ page, db }) => {
  ...
  test("Edit Currency Code", async ({ page, db }) => {
    const { uiVals } = await CurrencySetupEdit(
      page,
      sideMenu,
      paths,
      columns,
  ...
  test("Delete Currency Code", async ({ page, db }) => {
    ...
    if (dbValues.length > 0) {
      throw new Error("DB validation failed when deleting Currency Code");
    }
  });
