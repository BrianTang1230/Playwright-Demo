...
import {
  MillEquipmentSetupCreate,
  MillEquipmentSetupEdit1,
  MillEquipmentSetupEdit2,
  MillEquipmentSetupDelete,
} from "@UiFolder/pages/MasterFile/08_Oil_Mill_Production/01_MillEquipmentSetupPage";
...
const formName = "Mill Equipment Setup";
...
test.describe.serial(`${formName} Tests`, () => {
  ...
  test(`Create ${formName}`, async ({ page, db }) => {
    ...
    const { uiVals } = await MillEquipmentSetupCreate(
      page,
      sideMenu,
      paths,
      columns,
  ...
  test(`Edit ${formName} Without Saving`, async ({ page, db }) => {
    const { uiVals } = await MillEquipmentSetupEdit1(
      page,
      sideMenu,
      paths,
      columns,
  ...
  test(`Edit ${formName} With Saving`, async ({ page, db }) => {
    const { uiVals } = await MillEquipmentSetupEdit2(
      page,
      sideMenu,
      paths,
      columns,
  ...
  test(`Delete ${formName}`, async ({ page, db }) => {
    await MillEquipmentSetupDelete(page, sideMenu, editValues, ou);

    // Check if the Mill Equipment Setup is deleted
    const dbValues = await db.retrieveData(masterSQLCommand(formName), {
      Code: editValues[0],
      OU: ou[0],
    });
