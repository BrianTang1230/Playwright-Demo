...
import {
  StorageTankSetupCreate,
  StorageTankSetupEdit1,
  StorageTankSetupEdit2,
  StorageTankSetupDelete,
} from "@UiFolder/pages/MasterFile/08_Oil_Mill_Production/03_StorageTankSetupPage";
...
const formName = "Storage Tank Setup";
...
test.describe.serial(`${formName} Tests`, () => {
  ...
  test(`Create ${formName}`, async ({ page, db }) => {
    ...
    const { uiVals } = await StorageTankSetupCreate(
      page,
      sideMenu,
      paths,
      columns,
  ...
  test(`Edit ${formName} Without Saving`, async ({ page, db }) => {
    const { uiVals } = await StorageTankSetupEdit1(
      page,
      sideMenu,
      paths,
      columns,
  ...
  test(`Edit ${formName} With Saving`, async ({ page, db }) => {
    const { uiVals } = await StorageTankSetupEdit2(
      page,
      sideMenu,
      paths,
      columns,
  ...
  test(`Delete ${formName}`, async ({ page, db }) => {
    await StorageTankSetupDelete(page, sideMenu, editValues, ou);

    // Check if the Storage Tank Setup is deleted
    const dbValues = await db.retrieveData(masterSQLCommand(formName), {
      Code: editValues[0],
      OU: ou[0],
    });
