...
import {
  StockAdjustmentRemarkSetupCreate,
  StockAdjustmentRemarkSetupEdit1,
  StockAdjustmentRemarkSetupEdit2,
  StockAdjustmentRemarkSetupDelete,
} from "@UiFolder/pages/MasterFile/08_Oil_Mill_Production/04_StockAdjustmentRemarkSetupPage";
...
const submodule = "Oil Mill Production";
const formName = "Stock Adjustment Remark Setup";
...
test.describe.serial(`${formName} Tests`, () => {
  ...
  test(`Create ${formName}`, async ({ page, db }) => {
    ...
    const { uiVals } = await StockAdjustmentRemarkSetupCreate(
      page,
      sideMenu,
      paths,
      columns,
  ...
  test(`Edit ${formName} Without Saving`, async ({ page, db }) => {
    const { uiVals } = await StockAdjustmentRemarkSetupEdit1(
      page,
      sideMenu,
      paths,
      columns,
  ...
  test(`Edit ${formName} With Saving`, async ({ page, db }) => {
    const { uiVals } = await StockAdjustmentRemarkSetupEdit2(
      page,
      sideMenu,
      paths,
      columns,
  ...
  test(`Delete ${formName}`, async ({ page, db }) => {
    await StockAdjustmentRemarkSetupDelete(page, sideMenu, editValues);

    // Check if the Stock Adjustment Remark code is deleted
    const dbValues = await db.retrieveData(masterSQLCommand(formName), {
      Code: editValues[0],
    });
