...
import {
  ExternalBlockSetupCreate,
  ExternalBlockSetupEdit1,
  ExternalBlockSetupEdit2,
  ExternalBlockSetupDelete,
} from "@UiFolder/pages/MasterFile/09_Weighbridge/05_ExternalBlockSetupPage";
...
const formName = "External Block Setup";
...
test.describe.serial(`${formName} Tests`, () => {
  ...
  test(`Create ${formName}`, async ({ page, db }) => {
    ...
    const { uiVals } = await ExternalBlockSetupCreate(
      page,
      sideMenu,
      paths,
      columns,
  ...
  test(`Edit ${formName} Without Saving`, async ({ page, db }) => {
    const { uiVals } = await ExternalBlockSetupEdit1(
      page,
      sideMenu,
      paths,
      columns,
  ...
  test(`Edit ${formName} With Saving`, async ({ page, db }) => {
    const { uiVals } = await ExternalBlockSetupEdit2(
      page,
      sideMenu,
      paths,
      columns,
  ...
  test(`Delete ${formName}`, async ({ page, db }) => {
    await ExternalBlockSetupDelete(page, sideMenu, editValues, ou);

    // Check if the External Block Setup is deleted
    const dbValues = await db.retrieveData(masterSQLCommand(formName), {
      Code: editValues[0],
      OU: ou[0],
    });
