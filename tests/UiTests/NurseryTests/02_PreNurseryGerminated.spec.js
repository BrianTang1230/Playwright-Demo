...
} from "@UiFolder/pages/Nursery/02_PreNurseryGerminated";
...
test.describe.serial("Pre Nursery Germinated Tests", () => {
  ...
  // ---------------- Create Test ----------------
  test("Create Pre Nursery Germinated", async ({ page, db }) => {
    ...
    docNo = await editJson(
      JsonPath,
      formName,
      await page.locator("#txtPGNum").inputValue()
    );
  ...
  // ---------------- Edit Test ----------------
  test("Edit Pre Nursery Germinated", async ({ page, db }) => {
    const { uiVals } = await PreNurseryGerminatedEdit(
      page,
      sideMenu,
      paths,
      columns,
  ...
  // ---------------- Delete Test ----------------
  test("Delete Pre Nursery Germinated", async ({ page, db }) => {
