...
import {
  ...
  NurseryTransferRequisitionEdit1,
  NurseryTransferRequisitionEdit2,
...
test.describe.serial(`${formName} Tests`, () => {
  ...
  test(`Create ${formName}`, async ({ page, db }) => {
    ...
    docNo = await editJson(
      JsonPath,
      formName,
      await page.locator("#txtTransferNum").inputValue(),
    );
  ...
  test(`Edit ${formName} Without Saving`, async ({ page, db }) => {
    const { uiVals } = await NurseryTransferRequisitionEdit1(
      page,
      sideMenu,
      paths,
      columns,
  ...
  test(`Edit ${formName} With Saving`, async ({ page, db }) => {
    const { uiVals } = await NurseryTransferRequisitionEdit2(
      page,
      sideMenu,
      paths,
      columns,
